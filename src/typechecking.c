...
int correctFuncTypesReal(int line, symbol_t* curSymbol, string funcID,
		exprList_t* expressions, param_t* parameters, int expressionList) {
	...
	for (param_t* s = parametersHash; s != NULL; s = s->next) {
		if ((expressionList == 0 && parameters == NULL)
				|| (expressionList == 1 && expressions == NULL)) {

			typeError(
					line,
					"Too few parameters; %d parameters expected, but %d parameters found, is expression list: %d",
					function->num_params, i - 1, expressionList);
			printExpectedFound(line, parametersHash, expressionsOrg, parametersOrg, expressionList);
			return 0;
		}
		...
		if (s->var->type != paraExprType) {
			typeError(
					line,
					"Type of parameter %d is incompatible in function call %s; %s expected, but %s found, is expression list: %d",
					i, function->id, typeToString(s->var->type),
					typeToString(paraExprType), expressionList);
			return 0;
		}
		if (expressionList == 1) {
	...
	if ((expressionList == 0 && parameters != NULL)
		...
		typeError(
				line,
				"Too much parameters; %d parameters expected, but %d parameters found, is expression list: %d",
				function->num_params, i, expressionList);
		printExpectedFound(line, parametersHash, expressionsOrg, parametersOrg, expressionList);
		return 0;
	}
